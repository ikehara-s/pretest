...
(function(){
	...
	pushChoice('RMANカタログを使用せずに、正しい時刻が見つかるまで繰り返し実行できます。', true);
	pushChoice('フラッシュバックデータベースを機能させるには、データベースを有効にする必要があります。', false);
	pushChoice('DBAが管理する補助インスタンスを使用して実行できます。', false);
	...
	''));
	pushChoice('トランスポータブル表領域の操作中に、表領域が読み取り/書き込みモードのままになることはありません。', true);
	pushChoice('暗号化された列を持つテーブルの通常のデータポンプエクスポートでは、インポート時に常に同じ列が暗号化されます。', false);
	pushChoice('トランスポータブルデータポンプのインポートにより、プラグインされたテーブルスペースが読み取り専用モードのままになる場合があります。', false);
	...
	pushChoice('共通の統合監査ポリシーをアプリケーションレベルで作成できます。', false);
	pushChoice('CDBレベルで共通の統一監査ポリシーを作成できます。', false);
	pushChoice('アプリケーションPDBはローカル監査ポリシーを持つことができません。', true);
	pushChoice('アプリケーションルートで定義されたファイングレイン監査ポリシーは、アプリケーションルートに含まれる各アプリケーションPDBによって手動で同期する必要があります。', true);
	pushChoice('アプリケーションルートで定義されたアプリケーション共通の統合監査ポリシーは、アプリケーションルートに含まれる各アプリケーションPDBによって手動で同期する必要があります。', true);
	sortChoice();
}());
